    ...
    <section className="learning-tool" aria-labelledby="sample-practice-title">
      ...
      <div className="sample-practice-grid">
        ...
        <div className="sample-practice-panel">
          ...
          <button className="button button-dark" type="button" disabled={!prediction || saveState === "saving"} onClick={revealPrediction}><Eye /> Reveal the feedback</button>
          {activeFinding ? <article className="revealed-finding"><p className={`finding-priority priority-${activeFinding.priority}`}>{activeFinding.priority}</p><h4>{activeFinding.what}</h4><dl><div><dt>What we noticed</dt><dd>{activeFinding.evidence}</dd></div><div><dt>Why</dt><dd>{activeFinding.why}</dd></div><div><dt>First action</dt><dd>{activeFinding.how}</dd></div></dl><button className={progress.checkedActionIds.includes(activeFinding.actionId) ? "button-secondary is-complete" : "button-secondary"} type="button" onClick={() => void persist({ ...progress, checkedActionIds: Array.from(new Set([...progress.checkedActionIds, activeFinding.actionId])) })}><Check /> {progress.checkedActionIds.includes(activeFinding.actionId) ? "First fix chosen" : "Choose this first fix"}</button></article> : null}
