  ...
  return (
    <section className="learning-tool" aria-labelledby="sample-practice-title">
      ...
      <div className="sample-practice-grid">
        <figure className="learning-image-frame compact"><Image alt={sample.alt} height={sample.height} sizes="(max-width: 900px) 100vw, 46vw" src={sample.asset} width={sample.width} />{sample.regions.map((region, index) => progress.revealedFindingIds.some((id) => sample.findings.find((finding) => finding.id === id)?.regionId === region.id) ? <span aria-hidden="true" className="learning-region is-visible" key={region.id} style={{ left: `${region.x}%`, top: `${region.y}%`, width: `${region.width}%`, height: `${region.height}%` }}><b>{index + 1}</b></span> : null)}</figure>
