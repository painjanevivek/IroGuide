      ...
      {loading ? (
        ...
        <Reveal delay={0.08}>
          <div className="dashboard-empty is-empty">
            <div><LayoutDashboard size={38} /><h2>Your saved critiques will appear here.</h2><p>{liveCritique ? "Complete a personalized critique to build your private review history." : "Personalized critiques are temporarily paused; your learning artifacts and saved history remain available."}</p><Link className="button button-dark" href={(dashboardGuide.guide?.nextAction.href ?? reviewActionHref) as Route}>{dashboardGuide.guide?.nextAction.label ?? reviewActionLabel} <Sparkles /></Link></div>
          </div>
        </Reveal>
      ) : (
        <>
          ...
          {progress.totalReviews > 0 ? <><section aria-label="Design progress summary">
            <Stagger className="progress-grid">
              <StaggerItem><article><span>Comparable reviews</span><strong>{progress.totalReviews}</strong><p>{progressCohort.excludedCount > 0 ? `${progressCohort.excludedCount} incompatible or unverified excluded` : "Server-verified feedback cohort"}</p></article></StaggerItem>
              <StaggerItem><article className="metric-violet"><span>Average score</span><strong>{progress.totalReviews > 0 ? <>{progress.averageScore}<small>/10</small></> : "—"}</strong><p>{progress.totalReviews === 0 ? "No compatible verified feedback" : progress.scoreChange === null ? "Build a baseline with one more review" : `${progress.scoreChange >= 0 ? "+" : ""}${progress.scoreChange} since your first review`}</p></article></StaggerItem>
              <StaggerItem><article><span>Strongest area</span><strong className="metric-word">{progress.strongest?.label ?? "Awaiting feedback"}</strong><p>{progress.strongest ? `${progress.strongest.score}/10 compatible average` : "Needs 2 compatible reviews"}</p></article></StaggerItem>
          ...
          <Reveal delay={0.12}>
            <section className="learning-card"><Sparkles className="sparkle-blink-glow" /><div><span className="mono-label">VERIFIED LEARNING FEEDBACK</span><h2>{progress.evidenceState === "comparable" ? "One useful constraint." : "Build a trustworthy baseline."}</h2><p>{progress.evidenceState === "comparable" ? progress.lesson : progressCohort.reason}</p>{progress.insights.length > 0 && <ul className="insight-list">{progress.insights.map((insight) => <li key={insight}>{insight}</li>)}</ul>}{progress.recurringIssues.length > 0 && <ul className="insight-list">{progress.recurringIssues.map((issue) => <li key={issue.category}>{issue.category} recurred in {issue.count} compatible reviews.</li>)}</ul>}</div><Link href={reviewActionHref}>{liveCritique ? "Practice with a new design" : "Explore example critique"} <ArrowRight /></Link></section>
          </Reveal>
          </> : null}
          <Reveal delay={0.14}>
