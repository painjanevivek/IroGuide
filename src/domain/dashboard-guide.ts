...
export const dashboardGuideActionIds = [
  ...
  "start-brief", "continue-brief", "start-review", "request-access", "view-access", "continue-review-job", "continue-review-draft",
...
export function deriveDashboardGuide(input: GuideInput): DashboardGuide {
  ...
  const checklist = [
    ...
    { id: "inspect-sample" as const, label: "Inspect what you can see", outcome: "One first fix chosen from an owned sample", completed: sampleComplete, href: "/learn#practice" },
  ...
  if (input.activeReviewJob && input.liveCritique) {
    ...
    nextAction = action(sampleStarted ? "continue-sample" : "start-sample", "Step 2 of 4", sampleStarted ? "Continue where you left off." : "Practice feedback before opinion.", sampleStarted ? "Return to the finding you revealed and choose one useful first fix." : "Predict a finding, reveal the feedback, and decide what to change first.", "/learn#practice", sampleStarted ? "Continue sample" : "Start sample exercise", "Completed sample reflection");
    ...
    nextAction = action("continue-comparison", "Revision loop", "Continue the comparison you started.", "Return to the owned review and finish matching revision feedback.", "/dashboard#recent-reviews", "Continue comparison", "Saved comparison");
  } else if (input.caseStudyDraft) {
    state = "case-study-in-progress";
    nextAction = action("continue-case-study", "Private portfolio", "Continue the case study you started.", "Return to the private feedback-backed draft without publishing it.", "/portfolio", "Continue case study", "Private case study draft");
  } else if (input.reviewCount > 0) {
    state = "existing-reviews";
    nextAction = action("open-review-history", "Owned history", "Continue from your latest saved critique.", "Review the feedback and first action already attached to your account.", "/dashboard#recent-reviews", "Open recent critique", "Reviewed saved critique");
  } else if (input.liveCritique) {
    state = "brief-ready";
    nextAction = action("start-review", "Your brief is ready", "Get a personalized critique of your design.", "Upload your work, choose a critique style, and receive clear, actionable feedback. No invitation is required.", "/review/new", "Start personalized review", "Saved personalized critique");
  } else if (input.access?.status === "invited") {
    state = "invited";
    nextAction = action("view-access", "Availability update", "Your earlier access record is saved.", "Personalized critiques are public when live capacity is enabled; prior invitation state is no longer required.", "/learn?tool=access#practice", "View availability", "Saved availability state");
  } else if (input.access?.status === "interested") {
    state = "access-requested";
    nextAction = action("view-access", "Free path complete", "Your review-availability interest is recorded.", "No email or provider job is created. Personalized critiques will be open to every verified user when live capacity returns.", "/learn?tool=access#practice", "View availability", "Revocable availability interest");
  } else {
    state = "brief-ready";
    nextAction = action("request-access", "Free path complete", "You have a brief ready for a personalized critique.", "Live capacity is temporarily paused. Record optional category interest or keep practicing; no invitation will be required.", "/learn?tool=access#practice", "Review availability", "Revocable availability interest");
  }
  ...
  const activity = [
    ...
    ...(input.access ? [{ type: "access" as const, label: `Review availability ${input.access.status}`, category: input.access.preferredCategory, at: input.access.updatedAt }] : []),
